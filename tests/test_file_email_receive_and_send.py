...
def test_receive_email_write_json_and_hist(form, setup_and_cleanup):
    with patch('services.io_utils.filewriter.ARCHIVE_DIR', new='data/archive'), \
        ...
        MessengerFactory.get_messenger(MessengerOptions.EMAIL).receive_message(form)
...
def test_receive_email_queue_response(form, setup_and_cleanup):
...
def test_send_email_from_queue(form, setup_and_cleanup):
    timestamp = datetime.datetime.now()
